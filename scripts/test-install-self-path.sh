...
cp erpnext-dev.sh VERSION SHA256SUMS RELEASE-MANIFEST.txt "${fake_root}/"
...
install_self_for_reuse() {
  ...
  for metadata_name in BUILD-INFO.json SHA256SUMS SHA256SUMS.asc RELEASE-MANIFEST.txt; do
    if [[ -f "${src_root}/${metadata_name}" ]]; then
      cp -a "${src_root}/${metadata_name}" "${dest_root}/${metadata_name}" || return 1
    else
      rm -f "${dest_root}/${metadata_name}" || return 1
    fi
  done
...
printf '%s\n' '{"project_version":"1.20.2"}' >"${TOOLKIT_INSTALL_DIR}/BUILD-INFO.json"
printf '%s\n' '# stale sums' >"${TOOLKIT_INSTALL_DIR}/SHA256SUMS"
...
[[ ! -f "${TOOLKIT_INSTALL_DIR}/BUILD-INFO.json" ]] || note_fail "stale BUILD-INFO.json survived /opt refresh"
cmp -s "${fake_root}/SHA256SUMS" "${TOOLKIT_INSTALL_DIR}/SHA256SUMS" || note_fail "SHA256SUMS was not refreshed under /opt"
cmp -s "${fake_root}/RELEASE-MANIFEST.txt" "${TOOLKIT_INSTALL_DIR}/RELEASE-MANIFEST.txt" || note_fail "RELEASE-MANIFEST.txt was not refreshed under /opt"
