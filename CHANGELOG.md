...
## v1.20.4-beta.3 - Fix update-toolkit version option parsing
...
- Added a signed stable/beta one-command installer that resolves exact published
  releases, verifies the existing release trust chain without root, installs
  atomically, repairs the CLI, and safely hands the real terminal to first-run.
...
gate, and recovers installs left incomplete by the legacy single-file updater.

### Fixed

- **Release CI / ShellCheck:** `menu_render_test` exports `MENU_NO_CLEAR` for
  cross-module use in `lib/ui.sh` (SC2034). This was blocking `validate-release`
  on main after the UX commit.
- **Legacy single-file updater bridge:** when the canonical `/opt` entry script
  is modular but `lib/` is missing, recover once from the signed release bundle
  matching `SCRIPT_VERSION` (checksums + pinned GPG fingerprint + detached
  signature), promote into `releases/<version>`, switch `current`, and re-exec.
  Source checkouts stay fail-closed (no silent network recovery).
...
- **Interactive page lifecycle:** numbered menu selections clear only the live
  ...
  print-and-exit.
- **Result-page navigation:** shared pauses use `[Enter] Back` / `[q] Quit`.
- **High-use menus retain results** before returning to the parent menu.
- **Smaller-terminal two-column layout** with automatic one-column fallback.
- **Cleaner main menu labels** and focused credentials overview /
  `credentials-show` output.

### Validation

- Hermetic UI render tests and `scripts/test-legacy-modular-bootstrap.sh`.
...
- **Release CI cancel during shellcheck:** removed workflow `concurrency`
  cancel-in-progress (it aborted tag/main shellcheck with “operation was
  canceled”); CI installs upstream shellcheck; per-file timeouts;
  `validate-release` skips a duplicate pass (`SKIP_SHELLCHECK=1`).
...
operations dashboard. No auto-healing yet (monitor-only); healing and external
watchdog remain planned for v1.18 / v1.19. See
[`docs/HEALTH-ARCHITECTURE.md`](docs/HEALTH-ARCHITECTURE.md).
...
- Hermetic `scripts/test-health-snapshot.sh` for status-model aggregation.
...
- **Firefox-specific trust steps** in `browser-trust-guide` and the local HTTPS
  success screen. Firefox does not use the system trust store, so it can warn
  "Not Secure" even when Chrome/Brave/Edge already trust a mkcert site. The guide
  now covers: installing `certutil` (`libnss3-tools`) and re-running
  `mkcert -install`, the Snap/Flatpak-Firefox sandboxed-profile case
  (`security.enterprise_roots.enabled=true` or manual `rootCA.pem` import), and
  the reminder to fully restart Firefox.
