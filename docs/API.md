...
`api-version --json`, `capabilities --json`, and `deployment-info --json` are covered by the stable API contract. Other registry records may advertise `json_available` because they have legacy JSON output; they are not stable v1 commands unless `stable_api_contract` is `true` and `api_contract_version` is `"1.0"`.
...
`deployment-info` differs from future dashboard and health surfaces: it reports configuration intent only. Dashboard-style aggregation and health/readiness observations require separate commands with explicit runtime evidence and are outside this contract.
