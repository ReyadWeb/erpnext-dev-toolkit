# ERPNext Developer Toolkit Roadmap History

This file preserves the roadmap state immediately before the Phase D3 documentation
reconciliation following the v1.20.0 stable release.

It is an archive, not the active plan. Current priorities are maintained in
[`ROADMAP.md`](../ROADMAP.md). Shipped release history is authoritative in
[`CHANGELOG.md`](../CHANGELOG.md).

Some status statements and version assignments below are intentionally obsolete.
They are retained for design traceability and should not be interpreted as current
release guidance.
...
**Milestones / issues:** tracked on GitHub so progress stays visible (see [docs/ROADMAP-BOARD.md](ROADMAP-BOARD.md)).

**Full history:** [`CHANGELOG.md`](../CHANGELOG.md) · **Security:** [`SECURITY.md`](../SECURITY.md) · **Health model:** [`docs/HEALTH-ARCHITECTURE.md`](HEALTH-ARCHITECTURE.md) · **Testing:** [`TESTING.md`](../TESTING.md)
...
| Docker access + guided HTTPS + credentials parity | Stable (v1.19.20; local VM and real VPS validation passed) |
...
**Controls:** action registry, cooldowns, max-actions window, lockout after repeated failure, before/after incident records, recovery verification. See [`docs/HEALTH-ARCHITECTURE.md`](HEALTH-ARCHITECTURE.md).
...
**Status:** Shipped as **v1.19.10** (field validation recommended). See [`docs/FRAPPE-FRONTEND-ASSETS.md`](FRAPPE-FRONTEND-ASSETS.md).
...
### v1.19.15 — CLI Page UX + ShellCheck + modular recovery (P1)

**Status:** Shipped as **v1.19.15**. Epic [#108](https://github.com/ReyadWeb/erpnext-dev-toolkit/issues/108).

**Goal:** One interactive page lifecycle (clear → result → pause → parent) using existing `lib/ui.sh`; direct CLI stays print-and-exit; credentials become a focused secret page; unblock ShellCheck; recover legacy script-only `/opt` installs.

**Shipped**
- Numbered interactive selections clear only the live TTY; result pages pause with Enter/q.
