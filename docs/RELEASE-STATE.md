...
The metadata is generated by the release pipeline. It is not manually edited and
must agree with the tag, archive name, manifest, checksum inventory, and extracted
payload.
...
### R1B — Canonical runtime and channel implementation

- Remove the independent `SCRIPT_VERSION` literal.
- Make all workflows call `scripts/release-version.sh`.
- Derive channel from Git/build context.
- Generate and verify immutable bundle metadata.
- Advance the source tree to project version 1.20.1 development state.
- Enable invariant enforcement in CI.
