...
show_advanced_installation_menu() {
  while true; do
    ui_submenu_header "Advanced > Installation & Repair" \
      "Install, update, verify, repair, or intentionally remove the environment"
    print_two_column_menu \
      "1) Install / reinstall" \
      "2) Update toolkit" \
      "3) Verify toolkit" \
      "4) Repair CLI command" \
      "5) Repair environment" \
      "6) Installation preflight" \
      "7) Uninstall / reset [destructive]"
...
Core:
  ...
  protect-local-app-changes
                      Git-stash dirty app working trees before an ERPNext/Frappe upgrade
...
Local VM HTTPS / SSL:
  ...
  docker-reconcile-restart-policy Docker: repair persistent service reboot policies
...
main() {
  ...
  case "${ACTION:-menu}" in
    ...
    update-protect-local-changes | protect-local-app-changes | stash-app-changes) run_update_protect_local_changes ;;
    ...
    docker-reconcile-restart-policy) docker_reconcile_restart_policy ;;
