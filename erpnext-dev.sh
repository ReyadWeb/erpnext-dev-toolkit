...
Core:
  ...
  verify-frontend-assets  One-shot login CSS/JS (Link preload) probe
  wait-frontend-assets    Wait until ports + HTTP + static assets are OK
  repair-frontend-assets  bench build + clear-cache + restart + re-verify
