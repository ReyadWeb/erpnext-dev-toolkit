...
# Static metadata for existing public commands. Validation only; dispatch remains unchanged.
[[ -n "${_ERPNEXT_DEV_COMMANDS_LOADED:-}" ]] && return 0
_ERPNEXT_DEV_COMMANDS_LOADED=1

command_registry_records() {
  cat <<'EOF_COMMANDS'
menu|main|show_menu|user|read-only|none|interactive|safe|native,docker|no
...
health-check|health-check-status|show_health_check_status|user|read-only|none|both|safe|native,docker|yes
verify-toolkit|toolkit-verify,verify-install|verify_toolkit_integrity|user|read-only|none|both|safe|native,docker|no
update-toolkit|toolkit-update|update_toolkit|root|mutating|required|both|administrative|native,docker|no
toolkit-rollback|rollback-toolkit|rollback_toolkit|root|mutating|required|both|administrative|native,docker|no
backup-status|backup-state|show_backup_status|user|read-only|none|both|safe|native,docker|yes
restore-preflight|restore-check|show_restore_preflight|root|read-only|required|interactive|safe|native,docker|no
