...
# Maximum idle time for the one-shot create-site job. Each new log-progress
# marker refreshes this deadline so a slow but active ERPNext install is not
# reported as failed.
DOCKER_CREATE_SITE_TIMEOUT="${DOCKER_CREATE_SITE_TIMEOUT:-900}"
# Absolute safety ceiling for create-site, even when a noisy process continues
# producing output. This prevents a broken job from keeping setup open forever.
DOCKER_CREATE_SITE_MAX_TIMEOUT="${DOCKER_CREATE_SITE_MAX_TIMEOUT:-7200}"
...
docker_restart_policy_override_file() { printf '%s/erpnext-dev.restart-policy.yml\n' "$DOCKER_WORKDIR"; }
...
docker_compose_file_list() {
  ...
  if [[ -f "$(docker_restart_policy_override_file)" ]]; then
    printf '%s\n' "$(docker_restart_policy_override_file)"
  fi
}
...
# Persistent services must recover after a clean host reboot. Upstream
# frappe_docker uses `on-failure` in the disposable pwd.yml stack; Docker exits
# those containers with code 0 during a normal daemon shutdown, so they remain
# stopped on the next boot. Keep one toolkit-owned overlay for both development
# and production modes. One-shot configurator/create-site jobs are deliberately
# excluded.
docker_persistent_services() {
  printf '%s\n' \
    backend \
    db \
    frontend \
...
docker_write_restart_policy_override() {
  require_sudo
  local override
  override="$(docker_restart_policy_override_file)"

  $SUDO mkdir -p "$DOCKER_WORKDIR"
  $SUDO tee "$override" >/dev/null <<'EOF_DOCKER_RESTART_POLICY'
services:
  backend:
  ...
  websocket:
    restart: unless-stopped
EOF_DOCKER_RESTART_POLICY
  $SUDO chown root:root "$override" 2>/dev/null || true
  $SUDO chmod 644 "$override" 2>/dev/null || true
}

# Apply the policy to containers created by an earlier toolkit version without
# starting, stopping, or recreating them. The generated overlay keeps the same
# policy on future Compose recreations.
docker_reconcile_restart_policy() {
  ...
  if [[ ! -f "$(docker_compose_base_file)" && ! -f "$(docker_prod_base_file)" ]]; then
    warn "Docker stack is not provisioned; no restart policies were changed."
    return 0
  fi
  if ! docker_daemon_ready; then
    err "Docker daemon is not available; restart policies were not changed."
    return 1
  fi

  local svc cid updated=0
  docker_write_restart_policy_override
  while IFS= read -r svc; do
    [[ -n "$svc" ]] || continue
    cid="$(docker_compose ps -aq "$svc" 2>/dev/null | tail -n1 || true)"
    [[ -n "$cid" ]] || continue
    if ! ${SUDO:-} docker update --restart unless-stopped "$cid" >/dev/null; then
      err "Could not set restart policy for Docker service ${svc}."
      return 1
    fi
    updated=$((updated + 1))
  done < <(docker_persistent_services)

  # The production HTTPS proxy is not present in every Compose mode, so keep it
  # out of the static overlay and update it only when a container exists.
  cid="$(docker_compose ps -aq proxy 2>/dev/null | tail -n1 || true)"
  if [[ -n "$cid" ]]; then
    ${SUDO:-} docker update --restart unless-stopped "$cid" >/dev/null || {
      err "Could not set restart policy for Docker service proxy."
      return 1
    }
    updated=$((updated + 1))
  fi

  ok "Docker reboot policy reconciled (${updated} existing container(s); future services use unless-stopped)."
}
...
docker_now_epoch() { date +%s; }
docker_wait_sleep() { sleep "${1:-5}"; }

docker_create_site_container_result() {
  local cid="$1"
  ${SUDO:-} docker inspect \
    -f '{{.State.Status}}|{{.State.ExitCode}}' \
    "$cid" 2>/dev/null || printf 'unknown|1\n'
}

docker_create_site_progress_marker() {
  local cid="$1"
  ${SUDO:-} docker logs --timestamps --tail 1 "$cid" 2>/dev/null \
    | tr '\r' '\n' \
    | tail -n1
}

docker_create_site_finish_if_exited() {
  local result="$1" state code
  state="${result%%|*}"
  code="${result#*|}"
...
docker_wait_for_site_creation() {
  require_sudo
  local idle_timeout max_timeout started idle_deadline absolute_deadline
  local now cid="" result progress="" last_progress="" finish_rc

  idle_timeout="${DOCKER_CREATE_SITE_TIMEOUT}"
  max_timeout="${DOCKER_CREATE_SITE_MAX_TIMEOUT}"
  [[ "$idle_timeout" =~ ^[0-9]+$ && "$idle_timeout" -gt 0 ]] || idle_timeout=900
  [[ "$max_timeout" =~ ^[0-9]+$ && "$max_timeout" -gt 0 ]] || max_timeout=7200
  ((max_timeout < idle_timeout)) && max_timeout="$idle_timeout"

  started="$(docker_now_epoch)"
  idle_deadline=$((started + idle_timeout))
  absolute_deadline=$((started + max_timeout))
  log "Waiting for the create-site job to finish (site: $(docker_site_name); idle timeout: ${idle_timeout}s)"

  while :; do
    cid="$(docker_compose ps -aq create-site 2>/dev/null | tail -n1 || true)"
    now="$(docker_now_epoch)"

    if [[ -n "$cid" ]]; then
      result="$(docker_create_site_container_result "$cid")"
      if docker_create_site_finish_if_exited "$result"; then
        return 0
      else
        finish_rc=$?
        [[ "$finish_rc" -eq 1 ]] && return 1
      fi

      progress="$(docker_create_site_progress_marker "$cid" || true)"
    ...
    if [[ "$now" -ge "$absolute_deadline" || "$now" -ge "$idle_deadline" ]]; then
      # Close the race where create-site exits between the regular poll and the
      # timeout check. The final inspect decides success/failure before any
      # timeout is reported.
      if [[ -n "$cid" ]]; then
        result="$(docker_create_site_container_result "$cid")"
        if docker_create_site_finish_if_exited "$result"; then
      ...
      if [[ "$now" -ge "$absolute_deadline" ]]; then
        warn "create-site exceeded the absolute ${max_timeout}s safety limit. Recent create-site logs:"
      else
        warn "create-site made no observable progress for ${idle_timeout}s. Recent create-site logs:"
      fi
      docker_compose logs --no-color --tail 120 create-site 2>/dev/null || true
      return 1
    fi
...
# Return state, health, and restart policy for one Compose service. `ps -aq`
# intentionally includes stopped containers so Doctor can distinguish missing,
# exited, restarting, unhealthy, and healthy services.
docker_service_observation() {
  local svc="$1" cid
  cid="$(docker_compose ps -aq "$svc" 2>/dev/null | tail -n1 || true)"
  if [[ -z "$cid" ]]; then
    printf 'missing|none|none\n'
    return 0
  fi
  ${SUDO:-} docker inspect \
    -f '{{.State.Status}}|{{if .State.Health}}{{.State.Health.Status}}{{else}}none{{end}}|{{.HostConfig.RestartPolicy.Name}}' \
    "$cid" 2>/dev/null || printf 'unknown|none|unknown\n'
}

# Print a concise summary and return non-zero unless every required service is
# running and any configured Docker healthcheck is healthy.
docker_required_stack_status() {
  local svc observation state health policy total=0 ready=0
  local issues=""

  while IFS= read -r svc; do
    [[ -n "$svc" ]] || continue
    total=$((total + 1))
    observation="$(docker_service_observation "$svc")"
    IFS='|' read -r state health policy <<<"$observation"

    if [[ "$state" != "running" ]]; then
      issues+="${svc}=${state:-unknown}, "
      continue
    fi
    case "$health" in
      unhealthy | starting)
        issues+="${svc}=health-${health}, "
        continue
        ;;
    esac
    ready=$((ready + 1))
  ...
  if [[ "$ready" -eq "$total" && "$total" -gt 0 ]]; then
    printf '%s/%s required services ready\n' "$ready" "$total"
    return 0
  fi
  issues="${issues%, }"
  printf '%s/%s required services ready; %s\n' "$ready" "$total" "${issues:-state unavailable}"
  return 1
}

# A running stack can still be guaranteed to fail the next clean reboot when
# its containers retain upstream's `on-failure` policy. Keep this separate from
# runtime health so status can explain the persistence defect precisely.
docker_restart_policy_status() {
  local svc observation state health policy total=0 correct=0
  local issues=""
  ...
  while IFS= read -r svc; do
    ...
    if [[ "$policy" == "unless-stopped" ]]; then
      ...
      issues+="${svc}=${policy:-unknown}, "
    fi
  done < <(docker_persistent_services)

  if [[ "$correct" -eq "$total" && "$total" -gt 0 ]]; then
    printf '%s/%s persistent services use unless-stopped\n' "$correct" "$total"
    return 0
  fi
  issues="${issues%, }"
  printf '%s/%s persistent services use unless-stopped; %s; repair: %s\n' \
    "$correct" "$total" "${issues:-policy unavailable}" \
    "$(toolkit_cmd docker-reconcile-restart-policy)"
  return 1
}
...
docker_verify_access() {
  require_sudo
  local site domain code url probe_rc=1 stack_detail policy_detail
  local stack_rc=0 policy_rc=0
  site="$(docker_site_name)"
  domain="$(docker_public_domain)"
  ...
  if stack_detail="$(docker_required_stack_status)"; then
    status_line "Containers" "OK" "$stack_detail"
  else
    status_line "Containers" "FAIL" "$stack_detail; recover: $(toolkit_cmd start)"
    stack_rc=1
  fi

  if policy_detail="$(docker_restart_policy_status)"; then
    status_line "Reboot policy" "OK" "$policy_detail"
  else
    status_line "Reboot policy" "FAIL" "$policy_detail"
    policy_rc=1
  fi
...
docker_doctor_detail() {
  local compat svc observation state health policy check_status stack_detail policy_detail
  if docker_daemon_ready; then
    compat="daemon OK"
  elif docker_binary_present; then
  ...
  [[ "$compat" == "daemon OK" ]] || return 0
  [[ -f "$(docker_compose_base_file)" || -f "$(docker_prod_base_file)" ]] || return 0
  ...
  while IFS= read -r svc; do
    ...
    doctor_add_check "Docker service: ${svc}" "$check_status" \
      "state=${state:-unknown}; health=${health:-none}; restart=${policy:-unknown}"
  done < <(docker_persistent_services)

  if stack_detail="$(docker_required_stack_status)"; then
    doctor_add_check "Required Docker services" "OK" "$stack_detail"
  else
    doctor_add_check "Required Docker services" "FAIL" "$stack_detail"
  fi
  if policy_detail="$(docker_restart_policy_status)"; then
    doctor_add_check "Docker reboot policy" "OK" "$policy_detail"
  else
    doctor_add_check "Docker reboot policy" "FAIL" "$policy_detail"
  fi
}
